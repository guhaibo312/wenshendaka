//
//  ShopEditProcductViewController.m
//  Bshopkeeper
//
...
@interface ShopEditProcductViewController ()<UITextFieldDelegate,AddImageViewsDelegate,UIActionSheetDelegate,SelectedCommodityDelegate,UITextViewDelegate,UIAlertViewDelegate>

{
    ...
    UIScrollView *relateTattoo;             //关联纹身师
    NSArray *tattoolistArray;               //纹身师
...
@property (nonatomic, strong) ShopProductModelObject *productModel;

@property (nonatomic, strong) NSString *companyId;

@end

@implementation ShopEditProcductViewController
...
- (void)initWithSubViews
{
    ...
    JWEditView *relateItem = [[JWEditView alloc]initWithFrame:CGRectMake(0, desItem.bottom, SCREENWIDTH, 44) withTitleLabel:@"关联纹身师" type:JWEditLable detailImgName:@"icon_right_img.png"];
    relateItem.titleLabel.width = 120;
    [imageBottomView addSubview:relateItem];
    
    relateTattoo = [[UIScrollView alloc]initWithFrame:CGRectMake(100, 0, SCREENWIDTH-100-30, 44)];
    [relateItem setClickAction:@selector(selectedTattooController:) responder:self];

    relateTattoo.userInteractionEnabled = YES;
    [relateTattoo addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(selectedTattooController:)]];
    [relateItem addSubview:relateTattoo];
    
    
    
    UIView *middleView2  =[[UIView alloc]initWithFrame:CGRectMake(0, relateItem.bottom, SCREENWIDTH, 10)];
    ...
    completeBtn = [[FilledColorButton alloc]initWithFrame:CGRectMake(40, tagItem.bottom+40, SCREENWIDTH-80, 44) color:[UIColor whiteColor] borderColor:SEGMENTSELECT textClolr:[UIColor blackColor] title:@"保存" fontSize:16 isBold:NO];
...
- (void) deleteCommodity{
    ...
    [[JWNetClient defaultJWNetClient]deleteNetClient:@"CompanyProduct/info" requestParm:parm result:^(id responObject, NSString *errmsg) {
        [LoadingView dismiss];
        if (!self) return ;
        ...
        if (errmsg) {
            ...
            [[NSNotificationCenter defaultCenter]postNotificationName:shopProductChangedNotice object:nil];
            [self releaseAll];
...
- (void)clickCompleteBtnAction:(FilledColorButton *)sender
{
    ...
    if (tattoolistArray) {
        ...
        for ( int i = 0; i< tattoolistArray.count; i++) {
            NSDictionary *tempD = tattoolistArray[i];
            if ([NSObject nulldata:tempD[@"_id"]]) {
                [idArray addObject:tempD[@"_id"]];
            }
        }
        [parm setObject:idArray forKey:@"authorIds"];
    }
    if (_companyId) {
        [parm setObject:_companyId forKey:@"companyId"];
    }
...
- (void)requestAddFromServerWithHaveImgList:(NSArray *)imgList
{
    ...
    if (_isAdd) {
        [[JWNetClient defaultJWNetClient]putNetClient:@"CompanyProduct/info" requestParm:parm result:^(id responObject, NSString *errmsg) {
        ...
        [[JWNetClient defaultJWNetClient]postNetClient:@"CompanyProduct/info" requestParm:parm result:^(id responObject, NSString *errmsg) {
            [LoadingView dismiss];
            if (self == NULL) return ;
            completeBtn.enabled = YES;
            self.navigationItem.rightBarButtonItem.enabled = YES;
...
- (void)selectedTattooController:(id)sender
{
    ShopRelateTattooViewController *tattooVC = [[ShopRelateTattooViewController alloc]initWithQuery:@{@"id":_companyId?_companyId:@""}];
    tattooVC.selectedBlock = ^(NSArray *array){
      
        if (relateTattoo) {
            [relateTattoo.subviews makeObjectsPerformSelector:@selector(removeFromSuperview)];
        }
        int point_x = 10;
        for ( int i = 0 ; i< array.count; i++) {
            NSDictionary *dict = array[i];
            UIImageView *one = [[UIImageView alloc]initWithFrame:CGRectMake(point_x, 2, 40, 40)];

            [one sd_setImageWithURL:[NSURL URLWithString:dict[@"avatar"]] placeholderImage:[UIImage imageNamed:@"icon_userHead_default.png"]];
            [relateTattoo addSubview:one];
            one.layer.cornerRadius = 20;
            one.clipsToBounds = YES;
            point_x = one.right + 10;
        }
        
    };
    [self.navigationController pushViewController:tattooVC animated:YES];
}
