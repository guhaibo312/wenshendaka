...
@implementation JWRefreshBaseView
#pragma mark - 控件初始化
/**
 *  状态标签
 */
- (UILabel *)statusLabel
{
    if (!_statusLabel) {
        ...
        statusLabel.textColor = JWRefreshLabelTextColor;
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    frame.size.height = JWRefreshViewHeight;
...
- (void)layoutSubviews
{
    ...
    // 1.箭头
    CGFloat arrowX = self.JW_width * 0.5 - 100;
    self.arrowImage.center = CGPointMake(arrowX, self.JW_height * 0.5);
...
- (void)willMoveToSuperview:(UIView *)newSuperview
{
    ...
    // 旧的父控件
    [self.superview removeObserver:self forKeyPath:JWRefreshContentOffset context:nil];
    
    if (newSuperview) { // 新的父控件
        [newSuperview addObserver:self forKeyPath:JWRefreshContentOffset options:NSKeyValueObservingOptionNew context:nil];
        
        // 设置宽度
        self.JW_width = newSuperview.JW_width;
        // 设置位置
        self.JW_x = 0;
...
{
    if (self.state == JWRefreshStateWillRefreshing) {
...
- (BOOL)isRefreshing
{
    return JWRefreshStateRefreshing == self.state;
}
...
{
    if (self.state == JWRefreshStateRefreshing) {
        // 回调
        ...
        if (self.window) {
            ...
            _state = JWRefreshStateWillRefreshing;
...
- (void)setState:(JWRefreshState)state
{
    ...
    // 2.根据状态执行不同的操作
    switch (state) {
		case JWRefreshStateNormal: // 普通状态
        {
            if (self.state == JWRefreshStateRefreshing) {
                [UIView animateWithDuration:JWRefreshSlowAnimationDuration * 0.6 animations:^{
                ...
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(JWRefreshSlowAnimationDuration * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    // 再次设置回normal
                    _state = JWRefreshStatePulling;
                    self.state = JWRefreshStateNormal;
                });
                // 直接返回
                return;
