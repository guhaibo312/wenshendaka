...
@property (weak, nonatomic) JWRefreshHeaderView *header;
@property (weak, nonatomic) JWRefreshFooterView *footer;
@end
...
#pragma mark - 运行时相关
static char JWRefreshHeaderViewKey;
static char JWRefreshFooterViewKey;

- (void)setHeader:(JWRefreshHeaderView *)header {
    [self willChangeValueForKey:@"JWRefreshHeaderViewKey"];
    objc_setAssociatedObject(self, &JWRefreshHeaderViewKey,
                             header,
                             OBJC_ASSOCIATION_ASSIGN);
    [self didChangeValueForKey:@"JWRefreshHeaderViewKey"];
}

- (JWRefreshHeaderView *)header {
    return objc_getAssociatedObject(self, &JWRefreshHeaderViewKey);
}

- (void)setFooter:(JWRefreshFooterView *)footer {
    [self willChangeValueForKey:@"JWRefreshFooterViewKey"];
    objc_setAssociatedObject(self, &JWRefreshFooterViewKey,
                             footer,
                             OBJC_ASSOCIATION_ASSIGN);
    [self didChangeValueForKey:@"JWRefreshFooterViewKey"];
}

- (JWRefreshFooterView *)footer {
    return objc_getAssociatedObject(self, &JWRefreshFooterViewKey);
}
...
- (void)addHeaderWithCallback:(void (^)())callback
{
    // 1.创建新的header
    if (!self.header) {
        JWRefreshHeaderView *header = [JWRefreshHeaderView header];
...
{
    return self.header.state == JWRefreshStateRefreshing;
}
...
- (void)addFooterWithCallback:(void (^)())callback
{
    // 1.创建新的footer
    if (!self.footer) {
        JWRefreshFooterView *footer = [JWRefreshFooterView footer];
...
{
    return self.footer.state == JWRefreshStateRefreshing;
}
